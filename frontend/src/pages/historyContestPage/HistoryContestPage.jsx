...
function HistoryContestPage({ token, risk }){
    ...
    React.useEffect(() => {
        var url = `${process.env.REACT_APP_URL_LINK}/api/contest-history/get-contest-history/${contestId}/${setId}`
    ...
    React.useEffect(() => {
        if(myContest){
            var url = `${process.env.REACT_APP_URL_LINK}/api/quiz/contest-slip-history-result/${contestId}/${setId}?page=${page}&count=${count}`
            // Example POST method implementation:
            async function putData(url = '') {
                // Default options are marked with *
                const response = await fetch(url, {
            ...
            putData(url)
            .then(data => {
                    //console.log(JSON.parse(data.betslips));
    ...
    },[myContest, page, count]);

    React.useEffect(() => {
        if(myContest){
            var url = `${process.env.REACT_APP_URL_LINK}/api/quiz/full-slip-result/${contestId}/${myContest.contestCode}?history=${true}`;
            // Example POST method implementation:
            async function putData(url = '') {
                // Default options are marked with *
                const response = await fetch(url, {
...
export default connect(mapStateToProps)(HistoryContestPage);